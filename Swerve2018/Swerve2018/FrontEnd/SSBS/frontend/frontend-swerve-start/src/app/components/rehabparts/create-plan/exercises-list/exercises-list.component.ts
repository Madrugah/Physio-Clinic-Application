...
export class ExercisesListComponent implements OnInit {
  ...
  ngOnInit() {  
    ...
    //get exercises and populate table
  ...
  newArray: Exercise[]=[];
  onSearch(form:any){//filter table based on search term
  ...
  selectThisExercise(x) {//select this email
    this.uploaded.emit(x);
  }
    ngOnChanges(...args: any[]) {//listen for changes
      this.trySend()
  }  
  trySend(){//send exercise list to parent
...
export interface Exercise {//interface to represent exercise data
    _id:String;
    duration: Number;
    name: String;
    description: String;
