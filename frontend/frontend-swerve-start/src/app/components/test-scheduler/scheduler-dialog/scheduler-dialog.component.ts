...
export class SchedulerDialogComponent implements OnInit {
  ...
  constructor(private schedulerService: SchedulerService, private dialog: MatDialog, @Inject(MAT_DIALOG_DATA) private data: any) { 
    console.log('Data for dialog update: ');
    console.log(this.data);
    if(this.data.data.Physician==undefined){
      this.data.data.Physician =this.data.Physician;
    }
    if(this.data.data.state!=0 && this.data.data.state!=3){
      this.editable=true;
    }
