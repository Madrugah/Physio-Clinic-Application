...
router.route('/:exercise_id')
    ...
    .delete(function (request, response) {
        let excId = request.params.exercise_id;
        Exercise.findByIdAndRemove(request.params.exercise_id, function (error, deleted) {
                if (!error) {
                cleanseDeleted(excId);
                response.json({exercise: deleted});
                }
        });
    });
    
    function cleanseDeleted(excId){
        RehabilitationPlan.find({}, (err, plans) => {
            for (let plan of plans){ 
                let index = plan.exercises.indexOf(excId);
                if (index > -1) {
                    plan.exercises.splice(index, 1);
                    console.log('deleted an exercise form a test');
                    plan.save();
                    cleanseDeleted(excId);
                }
            }
