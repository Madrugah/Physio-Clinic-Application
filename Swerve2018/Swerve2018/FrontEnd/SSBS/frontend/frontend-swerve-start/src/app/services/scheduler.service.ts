...
export class SchedulerService {
  ...
  constructor(private http: Http, private physService: PhysicianManageProfileService) { }
  headers;
  // do POST request on creation
  createAppointment(callback_fun, form: any){//create an appointment
   ...
   getAppointments(callback_fun, token){//get appointments based on token
    ...
    //   //console.log(obj.appointments);
      ...
      getMyAppointments(callback_fun, token){//get my appointments by token
       this.headers = new Headers({ 'Content-Type': 'application/json' });
    ...
    //   //console.log(obj.appointments);
       ...
       for(let i = 0; i<obj.appointments.length; i++){
         ...
         this.physService.retrieveProfile1(obj.appointments[i].Physician).then((resp)=>{
           const parseProfile = JSON.parse(resp._body).data; 
           var physName = "Dr. ";
           physName = physName.concat(parseProfile.familyName);
           Apps[i].physFirstName = parseProfile.givenName;
           Apps[i].physLastName = physName;
           //get the name of the physician that corresponds to the specific appointment
         });
           Apps[i]={
                id: obj.appointments[i]._id,
                title: obj.appointments[i].title,
     ...
     getPhysicians(callback_fun){//get list of physicians
       this.headers = new Headers({ 'Content-Type': 'application/json' });
   ...
   getSingleAppointment(id, callback_fun){//get a single appointment by id
      this.headers = new Headers({ 'Content-Type': 'application/json' });
  ...
  deleteAppointment(callback_fun, id){//delete appointment by id
      this.headers = new Headers({ 'Content-Type': 'application/json' });
