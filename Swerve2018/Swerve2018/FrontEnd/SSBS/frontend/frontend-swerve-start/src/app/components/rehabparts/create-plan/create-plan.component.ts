...
export class CreatePlanComponent implements OnInit {
  ...
  ngOnInit() {  //authenticate user
    this.authServ.validate().then( (resp) => {
      ...
      if( !validateRaw.data && !validateRaw.data3){ // means they r nott admin
  ...
  selectThisExercise(x): void { 
    this.selectedExercises.push(x);//add exercise to selected list
  }
  
  selectThisAssessment(x): void { 
    // //console.log(x);
    this.selectedTests.push(x);//add test to selected list
  }

  removeTest (qToRemove): void { 
    //console.log(qToRemove);
    if (qToRemove > -1) {//remove test from selected list
  ...
  removeExercise (qToRemove): void { 
    if (qToRemove > -1) {//remove exercise from selected list
      this.selectedExercises.splice(qToRemove, 1);
    }
  }
  
   onNewRehabPlan(): void { //on click, take name fields and selected list and create a new rehab plan
    ...
    if(this.rehabPlan.name == undefined){
      ...
      this.rehabManage.addPlan(this.rehabPlan).then( (response) => {
        ...
        if(parsedResp.type){ // reached if type is true, successful, so notify and redirect
          this.snackBar.open('Plan successfully created', 'Ok',  {
            duration: 3000
          });
  ...
  redirectSelect(): void {//redirect to plan select
