        ...
        PatientProfile.findOne({ email: request.body.email }, (error, emailProfile) => {
            emailProfile.remove((err, editedProf) => {
                if (err) {
                    ...
                    response.json({
                        type: true,
                        data: "Successfully removed account!",
                        data2: editedProf
                    });
                }
            });
...
router.route('/adminsCreateProfile')
    .post(function(request, response) {
        console.log(request.body)
        PatientProfile.findOne({ email: request.body.email.toLowerCase() }, (err, emailProfile) => {
            if (emailProfile) {
...
router.route('/getPhysicians')
    .get(function(request, response) {
        PatientProfile.find({ $or: [ { isPhysician: true }, { isAdmin: true }] }, function(err, patientProfiles) {
            if (err) {
                response.json({
                    type: false,
                    data: "Error occured: " + err
                ...
                console.log("number of profiles "+patientProfiles.length)
                response.json({
        ...
        PatientProfile.find({}, function(err, patientProfiles) {
            if (err) {
                ...
                for (var i = 0; i < profileList.length; i++) {
                    if (profileList[i].givenName == null || profileList[i].givenName == "null" || profileList[i].givenName == "undefined" || profileList[i].givenName == undefined || profileList[i].isAdmin == true || profileList[i].isPhysician == true ||
...
router.route('/unviewed')
    .get(function(request, response) {
        console.log("unviewed hit")
        Message.find({viewed: false}, (error, messages) => {
            if (error) {
                console.log(error);
                response.json({
                    type: false,
...
router.route('/setViewed/:message_id')
    .get(function(request, response) {
